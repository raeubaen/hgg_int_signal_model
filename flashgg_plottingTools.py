...
def plotPdfComponents(ssf,_outdir='./',_extension='',_proc='',_cat=''):
  ...
  hists['final'].Scale(1/hists['final'].Integral() * 1600/ssf.nBins)
  ...
  if len(pdfs.keys())!=1:
    ...
    for k,v in pdfs.items():
      if pdfItr == 0:
        ...
        print("%s_%s_recursive_fraction_%s_%i"%(ssf.proc,ssf.cat,k,pdfItr+1))
        frac = ssf.Pdfs['final'].getComponents().getRealValue("%s_%s_recursive_fraction_%s_%i"%(ssf.proc,ssf.cat,k,pdfItr+1))
      # Create histogram with 1600 bins
      hists[k] = v.createHistogram("h_%s%s"%(k,_extension),ssf.xvar,ROOT.RooFit.Binning(1600))
      hists[k].Scale(1/hists[k].Integral()*frac* 1600/ssf.nBins)
